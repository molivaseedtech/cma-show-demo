...
function emptyCommunity() { return { version: 1, checkins: [], alerts: [] }; }

export async function readCommunity() {
  const file = communityFile();
  try {
    const value = JSON.parse(await fs.readFile(file, 'utf8'));
    return { ...emptyCommunity(), ...value, checkins: value.checkins || [], alerts: value.alerts || [] };
  } catch (error) {
...
export async function addCheckin(input = {}) {
  ...
  const checkin = { id: crypto.randomUUID(), name, city, region, platforms, createdAt: new Date().toISOString() };
...
export async function publicCommunity() {
  const community = await readCommunity();
  return { checkins: community.checkins.slice(0, 100), alerts: community.alerts.slice(0, 20) };
}
