...
export async function addComment(input = {}) {
  ...
  if (!episodeId || !name || !comment) throw Object.assign(new Error('Add your name and comment.'), { status: 400 });
  const community = await readCommunity();
  const value = { id: crypto.randomUUID(), episodeId, episodeTitle, name, comment, status: 'pending', createdAt: new Date().toISOString() };
